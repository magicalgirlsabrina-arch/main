...
#   3. Claude Code bridge launchd service (so other familiars can borrow Claude)
#
# Usage:
#   ./setup-familiar-host.sh <hub-tailscale-name>
...
HUB="${1:-}"
if [ -z "$HUB" ]; then
  echo "Usage: $0 <hub-tailscale-name>" >&2
...
# 3. Claude Code bridge (optional; only if claude CLI is installed)
if command -v claude >/dev/null 2>&1; then
  echo ""
  echo "🤖 Setting up Claude bridge..."
...
EOF
  ...
  echo "⚠️  Claude Code CLI not found — skipping the claude-bridge step."
